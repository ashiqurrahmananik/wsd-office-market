...
function profileName(profile: unknown, fallback = "—") {
  if (Array.isArray(profile)) {
    const first = profile[0] as ProfileShape | undefined;
    return first?.display_name || fallback;
  }
  const p = profile as ProfileShape | null | undefined;
...
export default async function AdminPage() {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-10">
    ...
    <section className="mt-10">
      ...
      <div className="card mt-4 overflow-x-auto"><table className="w-full min-w-[850px] text-left text-sm"><thead className="bg-orange-50"><tr><th className="p-4">Item</th><th>Seller</th><th>Price</th><th>Status</th><th>Type</th><th>Created</th></tr></thead><tbody>{(listings||[]).map((l:any)=><tr className="border-t border-stone-100" key={l.id}><td className="p-4 font-bold">{l.title}</td><td>{profileName(l.profiles)}</td><td>{l.price!=null?`৳${l.price}`:"—"}</td><td><span className="rounded-full bg-stone-100 px-2 py-1 text-xs font-bold">{l.status}</span></td><td>{l.bid_enabled?"🔨 Bid":""}{l.buy_now_enabled?" 🛒 Buy":""}</td><td>{new Date(l.created_at).toLocaleDateString()}</td></tr>)}</tbody></table></div>
    </section>
